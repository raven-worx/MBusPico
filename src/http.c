	...
	while (buffer_len < REQUEST_BUFFER_SIZE)
	{
		...
		*end = 0;
		...
		if (strstr(buffer,"GET ") == buffer) { // starts with GET
			if (strstr(buffer,"GET / ") == buffer) { // only support root path
				status_code = 200;
			}
			else {
				status_code = 404;
...
static int send_response(int socket, int statusCode)
{
	...
	LOG_D(TAG, "HTTP: sending response status code %d", statusCode);
		...
		case 200:
			{
				...
				size_t data_size = mbuspi_get_values_json(data_buffer);
...
static void run_server()
{
    ...
    {
        LOG_E(TAG, "Unable to bind socket: error %d", errno);
        return;
    }
    ...
    LOG_I(TAG, "Starting server at %s on port %u", ip4addr_ntoa(netif_ip4_addr(netif_list)), ntohs(listen_addr.sin_port));
    ...
    while (true)
    {
        ...
        if (conn_sock < 0)
        {
            LOG_E(TAG, "Unable to accept incoming connection: error %d", errno);
            return;
        }
...
void mbuspi_http_task(void* arg) {
	LOG_D(TAG, "mpuspi_http_task()");
