...
int main() {
	...
	result = xTaskCreate(mbuspico_http_task, "HTTP_Task", configMINIMAL_STACK_SIZE*6, NULL, tskIDLE_PRIORITY+1, &hHttpTask);
	#if MBUSPICO_LOG_LEVEL >= LOG_DEBUG
	...
	result = xTaskCreate(mbuspico_udp_task, "UDP_Task", configMINIMAL_STACK_SIZE*6, NULL, tskIDLE_PRIORITY+1, &hUdpTask);
	#if MBUSPICO_LOG_LEVEL >= LOG_DEBUG
